// src/app/cierre/CierreContent.jsx

"use client"; // CRÍTICO: Indica que este componente se ejecuta en el cliente

import { useEffect, useState, useCallback } from "react";
import { useSearchParams, useRouter } from "next/navigation";
import styles from "./CierrePage.module.css"; // Asumo que el CSS está en la misma carpeta
import { supabase } from "@/lib/supabaseClient";

// El componente principal, renombrado
export default function CierreContent() {
    const router = useRouter();
    const params = useSearchParams();
    const sessionId = params.get("sessionId"); // ⬅️ Este hook es el que causaba el error
    ...
    // Envolver la función en useCallback para el useEffect
    const finalizeSession = useCallback(async () => {
        ...
        try {
            // 1. Obtener los datos finales de la sesión
            ...
            // 2. Bloquear la sesión (lógica omitida por brevedad, asumo que es correcta)
            if (data.status !== 'REVEALED') {
                const { error: updateError } = await supabase
                    .from('game_sessions')
                    .update({ status: 'CLOSED', final_results_snapshot: { deck: finalDeck, p1: p1Sel, p2: p2Sel } })
                    .eq('id', sessionId);
                if (updateError) console.error("Error al bloquear Sesión:", updateError);
            }

            // 3. Bloquear QR y Aumentar Contador (lógica omitida por brevedad, asumo que es correcta)
            if (qrId) {
                const { data: qrData } = await supabase.from('qr_codes').select('scan_count').eq('id', qrId).single();
                const currentCount = qrData?.scan_count || 0;
                const { error: qrUpdateError } = await supabase
                    .from('qr_codes')
                    .update({ status: 'USED', session_id: sessionId, scan_count: currentCount + 1 })
                    .eq('id', qrId);
                if (qrUpdateError) console.error("Error al actualizar QR:", qrUpdateError);
            } else {
                console.warn("QR ID no encontrado...");
            }
    ...
    // useEffect depende de finalizeSession y sessionId
    useEffect(() => {
        finalizeSession();
    }, [finalizeSession]); // Eliminamos sessionId de aquí porque ya está en useCallback
